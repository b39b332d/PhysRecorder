...
int main() {
	...
	{
		std::cout << "WIC Encoder test\n";
		encoder::encoder_init();
		auto en_stream_p = encoder::add_stream(1280 * 2, 720);
		...
		for (int i = 0; i < 1200; i++) {
			encoder::encode(std::shared_ptr<void>(img.data, [](auto p) {}),  en_stream_p);
